...
from src.core.data_io_manager import DataIOManager
from src.core.data_mutator import DataMutator, StatisticalTest
from src.core.diff_history import DiffHistoryManager, OperationType
...
class DataHandler:
    ...
    def import_google_sheets(self, sheet_id: str, sheet_name: str, delimiter: str = ",", decimal: str = ".",
                             thousands: str = None, gid: str = None) -> pd.DataFrame:
    ...
    def refresh_google_sheets(self) -> pd.DataFrame:
        ...
        return self.import_google_sheets(sheet_id=params["sheet_id"], sheet_name=params["sheet_name"],
                                         delimiter=params["delimiter"], decimal=params["decimal"],
                                         thousands=thousands_param, gid=params["gid"])

    def create_empty_dataframe(self, rows: int, columns: int, column_names: List[str] = None,
    ...
    def _save_state_with_diff(self, old_df: pd.DataFrame, new_df: pd.DataFrame,
                              operation_type: Union[OperationType, str], params: Dict[str, Any],
                              operation_log_entry: Optional[Dict[str, Any]] = None) -> None:
    ...
    def _execute_macro_operation(self, op_type: str, kwargs: Dict[str, Any]) -> None:
        """
        Execute a single data macro operation on the current dataset instance

        :param op_type: The operation identifier string matching the action
        :param kwargs: Filtered dictionary of configuration arguments for the operation
        """
        if op_type == "filter":
            self.filter_data(
    ...
    def apply_pipeline_macro(self, macro_source: str | list) -> None:
        """
        Load and apply a sequential series of data operations.

        :param macro_source: The serialized macro string or list of operations
        :raises ValueError: If no active dataframe is currently loaded
        """
        if self.df is None:
            raise ValueError("No data loaded to apply")

        operations: list[dict[str, Any]] = self._history.load_pipeline_macro(macro_source)

        df_backup: DataFrame = self.df.copy(deep=False)
        log_backup: list[dict[str, Any]] = self._history.operation_log.copy()
        node_backup: str = self._history.current_node_id
        sort_state_backup: tuple | None = self.sort_state
        current_op_type: str = "Unknown"
        ...
        try:
            for i, op in enumerate(operations):
                ...
                if old_parent_id and old_parent_id in id_mapping:
                    target_parent: str = id_mapping[old_parent_id]
                ...
                self._execute_macro_operation(current_op_type, kwargs)
            ...
            logger.error(
                ...
                f"Failed on operation '{current_op_type}'. Error: {e}",
                exc_info=True
            )
            raise
    ...
    def _apply_changes(self, changed_df: pd.DataFrame, log_entry: Dict[str, Any],
                       new_sort_state: Optional[tuple] = None, old_df: Optional[pd.DataFrame] = None,
                       operation_type: Optional[Union[OperationType, str]] = None,
                       operation_params: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
    ...
    def filter_data(self, column: str = None, condition: str = None, value: Any = None,
        ...
        return self._apply_changes(changed_df, log_entry, old_df=old_df, operation_type=op_type,
    ...
    def aggregate_data(self, group_by: List[str], agg_config: Dict[str, Union[str, List[str]]],
    ...
    def preview_aggregation(self, group_by: List[str], agg_config: Dict[str, Union[str, List[str]]],
    ...
    def merge_data(self, right_df: pd.DataFrame, how: str, left_on: List[str], right_on: List[str],
    ...
    def bin_column(self, column: str, new_column_name: str, method: str, bins: Any, labels: List[str] = None,
                   right_inclusive: bool = True, drop_original: bool = False) -> pd.DataFrame:
        if self.df is None:
            raise ValueError("No data loaded")
        old_df = self.df.copy(deep=False)
