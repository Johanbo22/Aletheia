...
from PyQt6.QtWidgets import QFontComboBox, QFrame, QGraphicsDropShadowEffect, QHBoxLayout, QPushButton, QSizePolicy, \
    QSplitter, QStackedLayout, QToolBox, QVBoxLayout, QWidget
from matplotlib.backends.backend_qt import NavigationToolbar2QT as NavigationToolbar
from matplotlib.backends.backend_qtagg import FigureCanvasQTAgg as FigureCanvas

from icons import IconBuilder, IconType
from src.ui.components.plot_settings_panel import PlotSettingsPanel
from src.ui.widgets.DrawingOrderWidgets import DrawingOrderFloatingActionButton, DrawingOrderPopup

class PlotTabUI(QWidget):
    """
    Skeleton of the entire PlotStudio interface.
    Consists of a dual column layout with the FigureCanvas and the NavigationToolbar to the left
    PlotSettingsPanel to the right and a splitter down the middle.
    """
    ...
    def init_ui(self, canvas: FigureCanvas, toolbar: NavigationToolbar) -> None:
        ...
        icon_name = "macOSCmdLogo.svg" if is_mac else "windows_logo.svg"
        icon_path = Path.cwd() / "icons" / icon_name

        self.shortcut_button = QPushButton()
        self.shortcut_button.setObjectName("ShortcutViewerButton")
        self.shortcut_button.setIcon(QIcon(str(icon_path)))
        self.shortcut_button.setToolTip("View Keyboard && Mouse shortcuts")
        self.toolbar.addWidget(self.shortcut_button)
        ...
        self.save_plot_button.setShortcut(QKeySequence("Ctrl+Alt+S"))
