...
from src.ui.dialogs.LogHistoryPopup import LogHistoryPopup
...
class StatusBar(QStatusBar):
    ...
    def __init__(self) -> None:
        ...
        self.log_history: deque[str] = deque(maxlen=StatusBarConstants.MAX_LOG_HISTORY)
        self.action_timeout: int = StatusBarConstants.DEFAULT_ACTION_TIMEOUT_SEC
        self.last_action_time: Optional[datetime] = None
        self.popup: Optional[LogHistoryPopup] = None
    ...
    def log(self, message: str, level: Union[LogLevel, str] = LogLevel.INFO, action_type: Optional[str] = None,
        ...
        escaped_message: str = html.escape(display_message)
        html_log: str = f'<span style="color:#777777">[{timestamp}]</span> <span style="color:{color}">{escaped_message}</span>'
    ...
    def log_action(self, action: str, details: Optional[Dict[str, Any]] = None,
                   level: Union[LogLevel, str] = LogLevel.INFO) -> None:
        """
        Log a specific action to the terminal.
        This method can log details about an action to the logger.instance
        """
        if isinstance(level, str):
            try:
                level = LogLevel[level.upper()]
    ...
    def eventFilter(self, source: Any, event: QEvent | QMouseEvent) -> bool:
        """Intercept mouse clicks on the terminal to open the history popup."""
        if event.type() == QEvent.Type.MouseButtonPress and event.button() == Qt.MouseButton.LeftButton:
            if source is self.terminal:
