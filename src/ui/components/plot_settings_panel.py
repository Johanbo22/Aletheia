...
from PyQt6.QtWidgets import QCheckBox, QComboBox, QDoubleSpinBox, QFontComboBox, QGraphicsOpacityEffect, QGroupBox, \
    QHBoxLayout, QLabel, QLineEdit, QListWidget, QPushButton, QRadioButton, QSlider, QSpinBox, QTabWidget, QVBoxLayout, \
    QWidget
...
class PlotSettingsPanel(QWidget):
    ...
    def init_ui(self):
        ...
        self.custom_tabs.setCornerWidget(self.reset_button, Qt.Corner.TopRightCorner)

        # TAB 1: BASIC
        from src.ui.components.plot_tabs.general_settings_tab import GeneralSettingsTab
        ...
        # TAB 2: APPEARANCE
        from src.ui.components.plot_tabs.appearance_settings_tab import AppearanceSettingsTab
        ...
        # TAB 3: AXES
        from src.ui.components.plot_tabs.axes_settings_tab import AxesSettingsTab
        ...
        from src.ui.components.plot_tabs.legend_grid_settings import LegendGridSettingstab
        ...
        from src.ui.components.plot_tabs.customization_settings_tab import CustomizationSettingsTab
        ...
        # TAB 6: ANNOTATIONS
        from src.ui.components.plot_tabs.annotations_settings_tab import AnnotationsSettingsTab
        ...
        # TAB 7: GEO
        from src.ui.components.plot_tabs.geospatial_settings_tab import GeospatialSettingsTab
    ...
    def _get_tracked_widget(self) -> list:
        """Returns a list of tracked widgets for the reset to defaults functionality"""
        tracked = []

        # DO_NOT track widgets related to data states
        exclude = {
            id(getattr(self, 'x_column', None)),
            id(getattr(self, 'hue_column', None)),
            id(getattr(self, 'subset_combo', None)),
            id(getattr(self, 'secondary_y_column', None)),
            id(getattr(self, 'active_subplot_combo', None)),
            id(getattr(self, 'y_column', None)),
            id(getattr(self, 'z_column', None)),
            id(getattr(self, 'settings_search_input', None)),
            id(getattr(self, 'reset_button', None))
        }
        valid_types = (
            QCheckBox, QComboBox, QFontComboBox, QSpinBox, QDoubleSpinBox, QLineEdit, QSlider, QRadioButton,
            QListWidget,
            QPushButton, ToggleSwitch)
        for attr_name in dir(self):
            widget = getattr(self, attr_name)
            if isinstance(widget, valid_types):
                if id(widget) not in exclude:
                    if isinstance(widget, QPushButton) and not attr_name.endswith("_button"):
                        continue
                    if isinstance(widget, QPushButton) and "color" not in attr_name and "bg" not in attr_name:
                        continue
    ...
    def _capture_default_state(self) -> None:
        """Captures the state of registred settings"""
        self._default_state = {}

        for widget in self._get_tracked_widget():
            widget_id = id(widget)
            if isinstance(widget, (QCheckBox, ToggleSwitch)):
                self._default_state[widget_id] = widget.isChecked()
            elif isinstance(widget, QFontComboBox):
                self._default_state[widget_id] = widget.currentFont().family()
            elif isinstance(widget, QComboBox):
                self._default_state[widget_id] = widget.currentText()
            elif isinstance(widget, (QSpinBox, QDoubleSpinBox)):
                self._default_state[widget_id] = widget.value()
            elif isinstance(widget, QLineEdit):
                self._default_state[widget_id] = widget.text()
    ...
    def reset_to_defaults(self) -> None:
        """Resets all settings widget to the initial default values"""
        ...
        for widget in self._get_tracked_widget():
            ...
            try:
                ...
                widget_id = id(widget)
                if widget_id not in self._default_state:
                    continue

                default_val = self._default_state[widget_id]

                if isinstance(widget, (QCheckBox, ToggleSwitch)):
                    widget.setChecked(default_val)
                elif isinstance(widget, QFontComboBox):
                    widget.setCurrentFont(QFont(default_val))
                elif isinstance(widget, QComboBox):
    ...
    def _expose_general_tab_widgets(self) -> None:
        ...
        self.stacked_bars_check = self.basic_tab.stacked_bars_check
        self.y_stack = self.basic_tab.y_stack
    ...
    def _expose_annotations_tab_widgets(self) -> None:
        ...
        self.deselect_ref_line_button = ann_tab.deselect_ref_line_button

        # Reference Spans
        self.reference_spans_list = ann_tab.reference_spans_list
        self.add_ref_span_button = ann_tab.add_ref_span_button
        self.delete_ref_span_button = ann_tab.delete_ref_span_button
        self.clear_ref_spans_button = ann_tab.clear_ref_spans_button
        self.ref_span_type_combo = ann_tab.ref_span_type_combo
        self.ref_span_params_stack = ann_tab.ref_span_params_stack
        self.ref_span_ymin_spin = ann_tab.ref_span_ymin_spin
        self.ref_span_ymax_spin = ann_tab.ref_span_ymax_spin
        self.ref_span_xmin_spin = ann_tab.ref_span_xmin_spin
        self.ref_span_xmax_spin = ann_tab.ref_span_xmax_spin
        self.ref_span_color_button = ann_tab.ref_span_color_button
        self.ref_span_color_label = ann_tab.ref_span_color_label
        self.ref_span_alpha_spin = ann_tab.ref_span_alpha_spin
        self.ref_span_zorder_spin = ann_tab.ref_span_zorder_spin
        self.ref_span_label_input = ann_tab.ref_span_label_input
        self.deselect_ref_span_button = ann_tab.deselect_ref_span_button
        self.update_ref_span_button = ann_tab.update_ref_span_button
        ...
        self.deselect_annotation_button = ann_tab.deselect_annotation_button
