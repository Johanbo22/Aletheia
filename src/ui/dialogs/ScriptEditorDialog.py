...
import numpy as np
import pandas as pd
from PyQt6.QtCore import QEvent, QPoint, QSettings, Qt, pyqtSignal
from PyQt6.QtGui import QColor, QFont, QFontDatabase, QFontMetrics, QKeySequence, QShortcut, QTextCursor
...
class ScriptEditorDialog(QDialog):
    ...
    def eventFilter(self, source, event: QEvent) -> bool:
        if source == self.console_output and event.type() == QEvent.Type.KeyPress:
            ...
            if event.key() in (Qt.Key.Key_Return, Qt.Key.Key_Enter):
                ...
                command: str = raw_line.replace(">>>", "", 1).strip() if raw_line.startswith(
                    ">>>") else raw_line.strip()
    ...
    def init_ui(self, code):
        ...
        info_label = QLabel(
            "<b>Script</b><br>Edit the <code>create_plot(df)</code> function below. Click the 'Run' button to update the plot.")
        ...
        security_label = QLabel(
            "Warning: Executing untrusted Python code can be dangerous and can harm your computer\nOnly run code you trust.")
    ...
    def _load_syntax_theme(self) -> None:
        ...
        from src.ui.PythonHighlighter import SyntaxCategory, DefaultColorScheme
    ...
    def open_theme_settings(self) -> None:
        from src.ui.dialogs.SyntaxHighlightSettingsDialog import SyntaxHighlightSettingsDialog
        ...
        for var_name, var_value in self.console_namespace.items():
            ...
            if isinstance(var_value, pd.DataFrame):
                ...
                try:
                    ...
                    if isinstance(var_value, str) or (
                            isinstance(var_value, (list, tuple, set)) and len(var_value) <= 10):
    ...
    def on_run_clicked(self) -> None:
        ...
        try:
            ...
            global_signals.request_toast(
                "Syntax Error", f"Cannot run script due to syntax error:\n{syntaxerror}", ToastLevel.ERROR
            )
            return
