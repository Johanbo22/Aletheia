...
class FindReplaceDialog(QDialog):
        ...
        try:
            ...
            if not found_cursor.isNull():
                ...
                global_signals.request_toast(
                    "No matches", f"Result '{text}' not found", ToastLevel.WARNING
                )

        except Exception as err:
            global_signals.request_toast(
                "Error", "An error occurred during Find", ToastLevel.ERROR
            )
            global_signals.request_log(f"An error occurred during Find: {str(err)}", LogLevel.ERROR)
    ...
    def replace_all(self):
        try:
            ...
            global_signals.request_toast(
                "Result", f"Replaced {count} occurrences of {text} with {replacement}", ToastLevel.SUCCESS
            )
