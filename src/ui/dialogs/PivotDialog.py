...
from PyQt6.QtWidgets import QComboBox, QDialog, QGroupBox, QHBoxLayout, QLabel, QListWidget, QPushButton, QSplitter, \
    ...
    def update_preview(self):
        ...
        try:
            ...
            if isinstance(preview_df.columns, pd.MultiIndex):
                preview_df.columns = [
                    f"{str(column[0])}_{str(column[1])}" if len(column) > 1 and column[1] else str(column[0]) for column
    ...
    def validate_and_accept(self):
        ...
        if not index_cols:
            global_signals.request_toast(
                "Validation Error", "Please select at least one Index column", ToastLevel.ERROR
            )
            return

        if not value_cols:
            global_signals.request_toast(
                "Validation Error", "Please select at least one Value column", ToastLevel.ERROR
            )
            return
