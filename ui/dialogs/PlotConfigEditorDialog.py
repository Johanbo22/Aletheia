...
from PyQt6.QtGui import QCloseEvent, QColor, QFont, QKeySequence, QShortcut, QSyntaxHighlighter, QTextCharFormat, \
    QTextCursor, QTextDocument, QTextFormat
from PyQt6.QtWidgets import QColorDialog, QDialog, QFrame, QHBoxLayout, QInputDialog, QLabel, QMessageBox, QPushButton, \
...
class PlotConfigEditorDialog(QDialog):
    ...
    def __init__(self, theme_name: str, theme_content: Dict[str, Any], is_protected: bool = False,
        ...
        if is_protected:
            info_label.setText(
                f"Editing: <b>{theme_name}</b> (Read-Only Default)<br><i style='color:orange'>Changes must be saved as a new theme.</i>")
    ...
    def validate_json(self) -> None:
        ...
        try:
            ...
            self.validation_label.setText(
                f"Invalid JSON: {parse_error.msg} (Line: {parse_error.lineno}, Col: {parse_error.colno})")
    ...
    def handle_save(self):
        try:
            ...
            global_signals.request_toast(
                "Invalid JSON", f"Error parsing JSON data:\n{str(Error)}", ToastLevel.ERROR
            )
            global_signals.request_log(f"Error parsing JSON file: {str(Error)}", LogLevel.ERROR)
            return

        self.final_content = content

        if self.is_protected:
            name, ok = QInputDialog.getText(self, "Save theme as", "Enter new theme name:",
                                            text=f"{self.theme_name}_Copy")
            if ok and name:
