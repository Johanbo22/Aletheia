...
class Animation(HelpAnimationEngine):
    """
    Animation showing the duplication of a column.
    Visualizes selecting a target column and expanding the table horizontally
    to insert an exact, independent copy of the data.
    """
    ...
    def __init__(self, parent=None) -> None:
        ...
        self.columns = [
            {"name": "ID", "is_target": False, "is_copy": False},
            {"name": "Value", "is_target": True, "is_copy": False},
            {"name": "Value_copy", "is_target": False, "is_copy": True},
            {"name": "Status", "is_target": False, "is_copy": False}
        ]
        ...
        self.base_col_w = 100.0
        self.row_h = 45.0

    def draw_animation(self, painter: QPainter, progress: float) -> None:
        p_intro = self.get_eased_progress(progress, 0.0, 0.1)
        p_highlight = self.get_eased_progress(progress, 0.15, 0.30)
        p_transform = self.get_eased_progress(progress, 0.40, 0.70)
        ...
        self._draw_table(painter, start_x, start_y, col_widths, p_highlight, p_transform, p_outro)
    ...
    def _draw_table(self, painter: QPainter, x: float, y: float, col_widths: list[float], p_highlight: float,
                    p_transform: float, p_outro: float) -> None:
        curr_x = x
        for c_idx, col in enumerate(self.columns):
            ...
            if col["is_target"]:
                ...
                highlight_intensity = 1.0 - p_outro
                header_bg = self.lerp_color(self.c_header_bg, self.c_highlight_bg, highlight_intensity)
                cell_bg = self.lerp_color(self.c_table_bg, self.c_highlight_bg, highlight_intensity)
            else:
            ...
            if col["is_copy"]:
                text_alpha = int(255 * p_transform)
            else:
                text_alpha = 255

            self._draw_header_cell(painter, curr_x, y, w, col["name"], header_bg, text_alpha)
            self._draw_row_cells(painter, curr_x, y, w, c_idx, cell_bg, text_alpha)
    ...
    def _draw_explanation(self, painter: QPainter, y: float, p_highlight: float, p_transform: float) -> None:
        ...
        if p_transform == 1.0:
            text = "Copy created. Original data remains unchanged."
        elif p_highlight == 1.0 and p_transform > 0.0:
            text = "Duplicating column..."
        elif p_highlight > 0.0:
            text = "Selecting column to duplicate..."
        else:
            text = "Raw Dataset"
